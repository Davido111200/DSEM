...
class ShakespeareProbInferenceForStyle(BaseProbInference):
    ...
    def dataset_signature(self):
        return {
            "sample": ("shakespeare", None, "train"),
            "result": ("shakespeare", None, "test")
        }

    def dataset_preprocess(self, raw_data):
        data = []
        for e in raw_data:
            query = (e["modern"], e["original"])
    ...
    def paralell_style_promptify(self, query, return_reference = False, Instruction = ''):
        modern, original = query
        with_sentence_and_paraphrase = Instruction + f'Original: "{modern}"; Paraphrased: "{original}"'
        with_sentence = Instruction + f'Original: "{modern}"; Paraphrased: "'

        if return_reference:
            return with_sentence, with_sentence_and_paraphrase, original
        else:
